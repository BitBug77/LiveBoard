  ...
  useEffect(() => {
    const verifyEmail = async () => {
      ...
      try {
        ...
        const response = await axios.get(`http://127.0.0.1:5000/api/auth/verify/${token}`);
        
        // If we get here, verification was successful
        setStatus('success');
        
        // Redirect to whiteboard after a short delay (to show success message)
        setTimeout(() => {
          router.push('/login');
        }, 2000);
      ...
      <div className="max-w-md w-full space-y-8 p-10 bg-white rounded-xl shadow-md">
        ...
        {status === 'success' && (
          <div className="text-center">
            ...
            <p className="mt-4 text-sm text-gray-600">
              Your email has been successfully verified. Redirecting you to the whiteboard...
            </p>
          </div>
        )}
